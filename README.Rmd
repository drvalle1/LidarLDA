...
We start by showing how to fit the model based on simulated data with 5 clusters. The simulated datasets contain information for 2,601 pixels (rows) and 30 height bins (columns), labeled z1, z2, ..., z30. 
...
Because this is based on simulated data, there is a nice pattern regarding how the relative abundance of each cluster changes as a function of topography. This is shown below.
...
If we have LiDAR data for the same area but collected in another year (i.e., t=2), we could have simply re-run LidarLDA on the new dataset. The problem with this approach is that it might have identified a different set of clusters, making temporal comparisons challenging. To avoid this problem, we rely on the folding-in operation to compare how the relative abundance of each cluster has changed through time. 
...
```{r, echo=F}
#using pre-computed results stored as internal data
Model.Results.t2=list(llk=LidarLDA:::llk_t2$x,
                      theta=LidarLDA:::theta_t2)
```
...
By spatially visualizing the relative abundances of each cluster at time t=2, we can notice that there has been substantial spatial change. For example, all the clusters have moved upward, with expansion of the cluster that dominated the lowlands (i.e., green cluster) and shrinkage of the highland area dominated by the orange cluster.
